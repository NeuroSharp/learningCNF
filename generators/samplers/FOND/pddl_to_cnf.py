...
import os
from samplers.FOND.pddl_parser import Parser
...
class CNF:
    ...
    def generateInputSat(self, nameFile, cnf_output):
        self.file_formula.close()
        with open(nameFile, 'r') as formula:
            # name_final = nameFile + 'header'
            with open(cnf_output, 'w') as final_formula:
...
def toCNF(path_domain, path_instance, cnf_output, iter_cnt) :
    p = Parser()
    p.set_domain(path_domain)
    p.set_problem(path_instance)


    rand_suffix = "a" # use path instance here
    name_formula_file = cnf_output + '.tmp'
    name_formula_file_extra = cnf_output + '-formula-extra.txt'
    name_sas_file = cnf_output + '.sas'
    ...
    controllerStates = generateControllerStates(iter_cnt)
    cnf.reset()
    ...
    # name_final =
    cnf.generateInputSat(name_formula_file, cnf_output)
    os.remove(name_formula_file)
    os.remove(name_formula_file_extra)
    os.remove(name_sas_file)
    # print('Done creating formula.')
...
#     args_parser.add_argument('-iter_cnt', type=int, default = 1, help = 'Number of iterations to unroll.')
...
#     toCNF(params['path_domain'], params['path_instance'], params['iter_cnt'])
