...
def process(fname, pq):
    ...
    try:
        log.info("Reading: {} (pid: {})".format(fname, os.getpid()))
        f1 = CNF()
        f1.from_file(os.path.join(dir_path, fname))
        m.append_formula(f1.clauses)

        gc_count = 0
        log.info("Solving: {} (pid: {})".format(fname, os.getpid()))
        res = m.solve()
        log.info("Solved:  {} (pid: {}) -> {}".format(fname, os.getpid(), res))
    except Exception as e:
        log.exception("Aborting {} (pid: {}) due to exception: {}".format(fname, os.getpid(), e))
        pq.put("");
        return
    else:
        if (gc_count <= 1): # Low GC calls, drop the instance
            log.info("Aborting {} (pid: {}) due to low # of GC calls.".format(fname, os.getpid()))
            pq.put("");
            return
    ...
    for lbdt in range(range_min, range_max):
        ...
        try:
            ...
            log.info("Solving: {} (pid: {}, thresh: {})".format(fname, os.getpid(), lbd_thresh))
            res = m.solve()
            log.info("Solved:  {} (pid: {}, thresh: {}) -> {}".format(fname, os.getpid(), lbd_thresh, res))
        except Exception as e:
            log.exception("Aborting {} (pid: {}, thresh: {}) due to exception: {}".format(fname, os.getpid(), lbd_thresh, e))
...
# def nothing():
    ...
    rewards = manager.list() # rewards list
    ...
    jobs = []
    for file in files:
        job = pool.apply_async(process, (file, pq))
        jobs.append(job)
    ...
    log.info("done! Plotting the results...")
    print(rewards)
