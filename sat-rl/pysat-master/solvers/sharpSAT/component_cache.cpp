...
void ComponentCache::init(Component &super_comp) {
    if (!config_.quiet)
        std::cout << sizeof(CacheableComponent) << " " << sizeof(mpz_class) << std::endl;
    ...
    if (statistics_.maximum_cache_size_bytes_ > free_ram) {
        std::cout << std::endl << " WARNING: Maximum cache size larger than free RAM available" << std::endl;
        std::cout << " Free RAM " << free_ram / 1000000 << "MB" << std::endl;
    }

    if (!config_.quiet) {
        std::cout << "Maximum cache size:\t"
                  << statistics_.maximum_cache_size_bytes_ / 1000000 << " MB" << std::endl
                  << std::endl;
    }
