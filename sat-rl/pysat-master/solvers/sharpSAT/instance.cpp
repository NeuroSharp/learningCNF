...
void Instance::compactClauses() {
    ...
    for (auto ofs : clause_ofs) {
        ...
        if (*it != SENTINEL_LIT) {
            ...
            num_clauses++;
            getHeaderOf(new_ofs).set_id(num_clauses);
...
void Instance::compactVariables() {
    ...
    r_var_map_.resize(variables_.size(), 0);
    unsigned last_ofs = 0;
    ...
    for (unsigned v = 1; v < variables_.size(); v++)
        if (isActive(v)) {
            ...
            last_ofs++;
            var_map[v] = last_ofs;
            r_var_map_[last_ofs] = v;
        }

    // printf("r_var_map_[");
    // for(int i=0;i<r_var_map_.size();i++) {
    //     printf("%i, ", r_var_map_[i]);
    // }
    // printf("]\n");
...
bool Instance::createfromFile(const std::string &file_name) {
    ...
    int lit;
    unsigned max_ignore = 1000000; // max num of chars until we reach 'p'
    ...
    if (!(input_file >> idstring && idstring == "cnf" && input_file >> nVars
            && input_file >> nCls)) {
        std::cerr << "Invalid CNF file" << std::endl;
        exit(0);
    }
