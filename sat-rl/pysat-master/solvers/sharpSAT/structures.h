...
#ifndef SharpSAT_STRUCTURES_H_
#define SharpSAT_STRUCTURES_H_
...
public:
    ...
    unsigned toIndex() const { return value_ - 2;}
...
class Literal {
public:
    std::vector<LiteralID> binary_links_ = std::vector<LiteralID>(1, SENTINEL_LIT); // Other literals that this literal appears in a binary clause with
    std::vector<ClauseOfs> watch_list_ = std::vector<ClauseOfs>(1, SENTINEL_CL); // The clauses that this literal is a watch for
...
// Maybe we can add clause features here to the header
class ClauseHeader {
