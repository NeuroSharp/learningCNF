...
def compile_solver(solver):
    ...
    """
    C_includes  = distutils.sysconfig.get_python_inc()
    np_includes = numpy.distutils.misc_util.get_numpy_include_dirs()[0]
    print('compiling {0}'.format(solver))
    os.system('cd solvers/{0} && make INCLUDES+=-I{1} INCLUDES+=-I{2} && cd ../..'
        .format(solver, C_includes, np_includes))
